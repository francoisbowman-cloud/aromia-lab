  ...
  return (
    <main className="bg-[#fbf8f3] text-ink dark:bg-[#0f0c09]">
      <section className="mx-auto max-w-[1440px] px-6 pb-12 pt-10 lg:px-10 lg:pb-16 lg:pt-14">
        <div className="flex items-center gap-4 border-b border-line pb-4 font-plex text-[9px] uppercase tracking-[.2em] text-muted"><span>Discovery result</span><span className="h-px flex-1 bg-line"/><span>Perfil / selección</span></div>
        <div className="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-[1.15fr_.85fr] lg:items-end">
          ...
          <div className="lg:justify-self-end"><p className="max-w-[42ch] font-sans text-[15px] leading-7 text-muted">{perfil.descripcion}</p><Link href="/quiz" className="mt-7 inline-block border-b border-ink pb-1 font-plex text-[9px] uppercase tracking-[.14em] text-ink transition hover:border-gold hover:text-gold-contrast">Rehacer lectura →</Link></div>
        </div>
      </section>

      <section className="mx-auto max-w-[1440px] px-6 pb-16 lg:px-10 lg:pb-24">
        <div className="mb-8 flex items-center gap-4 border-b border-line pb-4 font-plex text-[9px] uppercase tracking-[.18em] text-muted"><span>Selección recomendada</span><span className="h-px flex-1 bg-line"/><span>{recomendados.length} objetos</span></div>
        {recomendados.length > 0 ? (
          <div className="grid grid-cols-1 border-b border-line sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{recomendados.map((perfume, index) => <PerfumeCard key={perfume.slug} perfume={perfume} index={index} />)}</div>
        ) : (
