...
export default async function MagazineArticlePage({ params }: { params: { slug: string } }) {
  ...
  return (
    <main>
      ...
      <div className="border-b border-line"><div className="mx-auto flex h-[66px] max-w-[1440px] items-center justify-between px-6 lg:px-10"><Link href="/magazine" className="font-sans text-[12px] uppercase tracking-[.12em]">← Volver al Magazine</Link></div></div>
      ...
      <section className="mx-auto max-w-[1180px] px-6 py-10 lg:px-10 lg:py-14">
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-[minmax(220px,.55fr)_minmax(0,1.45fr)] lg:gap-20">
          ...
          {articulo.contenido_html ? <article className="prose prose-neutral max-w-[760px] font-display text-[19px] leading-[1.75] text-ink prose-headings:font-display prose-headings:text-ink prose-p:text-ink prose-blockquote:border-gold prose-blockquote:text-ink md:text-[22px]" dangerouslySetInnerHTML={{ __html: articulo.contenido_html }} /> : null}
        </div>
      </section>
