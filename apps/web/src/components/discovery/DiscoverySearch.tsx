...
export function DiscoverySearch({ perfumes, articles }: { perfumes: Perfume[]; articles: Article[] }) {
  const [query, setQuery] = useState("");
  const q = query.trim();
  const perfumeResults = useMemo(() => q ? perfumes.map((perfume) => ({ perfume, score: discoveryTextScore(perfume, q) })).filter((item) => item.score > 0).sort((a, b) => b.score - a.score).slice(0, 12) : [], [perfumes, q]);
  ...
  return <div>
    ...
    {!q ? <div className="py-16"><p className="max-w-[24ch] font-display text-4xl leading-[1.02] text-ink">Empieza por una sensación, una nota o un nombre.</p><p className="mt-4 max-w-[48ch] font-sans text-sm leading-6 text-muted">La búsqueda cruza el catálogo y Magazine para convertir una intención vaga en rutas concretas de descubrimiento.</p></div> : <>
