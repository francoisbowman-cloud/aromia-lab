  ...
  return (
    <div>
            ...
            <div>
              ...
              <div className="mt-7 space-y-5">
                {families.length ? families.map(([name, score], index) => (
                  <Link key={name} href={`/catalogo?familia=${encodeURIComponent(name)}`} className="group block min-h-11 py-1 outline-none focus-visible:ring-2 focus-visible:ring-gold/70">
      ...
      <div className="flex flex-col gap-4 py-8 sm:flex-row sm:items-end sm:justify-between">
        ...
        <button type="button" onClick={() => { clearDiscoveryProfile(); setProfile(loadDiscoveryProfile()); }} className="min-h-11 self-start px-1 font-plex text-xs uppercase tracking-[.12em] text-muted transition-colors hover:text-ink sm:self-auto">Reiniciar mi mapa</button>
      </div>
    </div>
  );
}
