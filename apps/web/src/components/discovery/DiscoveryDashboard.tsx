...
export function DiscoveryDashboard({ perfumes }: { perfumes: Perfume[] }) {
  ...
  const atlasFamilies = useMemo(
    () => Array.from(new Set(perfumes.map((p) => p.familia_olfativa).filter((value): value is string => Boolean(value)))).slice(0, 7),
    [perfumes],
  );
    ...
    <div>
      <section aria-labelledby="atlas-title" className="border-y border-line py-10 sm:py-12 lg:py-16">
        <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:gap-20">
          <div>
            <p className="font-plex text-xs uppercase tracking-[.16em] text-[#5a6b54] dark:text-[#b8c5b3]">Atlas olfativo</p>
            ...
            <p className="mt-6 max-w-[35ch] font-sans text-base leading-7 text-muted">Empieza por una familia real del catálogo. La selección cambia el campo de objetos; no inventa afinidades ni porcentajes.</p>
          </div>

          <div>
            <div className="flex flex-wrap gap-x-5 gap-y-3 border-t border-line pt-4">
              <button type="button" onClick={() => setAtlasFamily("")} className={`inline-flex min-h-11 items-center font-display text-[24px] leading-none transition sm:text-[30px] ${atlasFamily === "" ? "text-[#5a6b54]" : "text-muted hover:text-ink"}`}>Todo</button>
              {atlasFamilies.map((family) => (
                <button key={family} type="button" onClick={() => { setAtlasFamily(family); trackEvent("intention_discovery", { signal_type: "family", value: family, source: "atlas" }); }} className={`inline-flex min-h-11 items-center font-display text-[24px] capitalize leading-none transition sm:text-[30px] ${atlasFamily === family ? "text-[#5a6b54]" : "text-muted hover:text-ink"}`}>{family}</button>
              ))}
            </div>
          ...
          <div className="grid gap-12 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="mt-7 flex min-h-[150px] flex-wrap content-start gap-x-4 gap-y-4 border-t border-line pt-5">
                {notes.length ? notes.map(([name, score]) => <Link key={name} href={`/buscar?q=${encodeURIComponent(name)}`} onClick={() => trackEvent("olfactory_note_open", { note: name, source: "discovery_map" })} className="inline-flex min-h-11 items-center font-display capitalize leading-none text-ink transition-colors hover:text-[#5a6b54]" style={{ fontSize: `${17 + 10 * strength(score, noteMax)}px`, opacity: .62 + .38 * strength(score, noteMax) }}>{label(name)}</Link>) : <p className="font-sans text-sm leading-6 text-muted">Las notas aparecen cuando exploras pirámides verificadas.</p>}
              </div>

              <div className="mt-9 border-t border-line pt-5"><p className="font-plex text-xs uppercase tracking-[.14em] text-muted">Perfumistas</p><div className="mt-3">{perfumers.length ? perfumers.map(({ item, slug }) => <Link key={slug} href={`/perfumistas/${slug}`} onClick={() => trackEvent("perfumer_open", { perfumer_slug: slug, source: "discovery_map" })} className="flex min-h-11 items-center font-display text-xl text-ink transition-colors hover:text-[#5a6b54]">{item?.name ?? label(slug)}</Link>) : <p className="mt-4 font-sans text-sm leading-6 text-muted">Las autorías se incorporan al visitar perfiles verificados.</p>}</div></div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section aria-labelledby="routes-title" className="py-16 lg:py-24">
        <div className="mb-12 grid gap-6 lg:grid-cols-[1fr_.7fr] lg:items-end"><div><p className="font-plex text-xs uppercase tracking-[.14em] text-[#5a6b54] dark:text-[#b8c5b3]">Rutas posibles</p><h2 id="routes-title" className="mt-4 max-w-[10ch] font-display text-[42px] leading-[.92] tracking-[-.04em] sm:text-[52px] lg:text-[60px]">Perfumes que continúan la conversación.</h2></div><p className="max-w-[42ch] font-sans text-base leading-7 text-muted lg:justify-self-end">Cada ruta parte de señales concretas de tu recorrido; no inventa atributos nuevos.</p></div>
