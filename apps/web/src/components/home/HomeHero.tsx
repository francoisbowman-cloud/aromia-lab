    ...
    <section className="relative overflow-hidden border-b border-line bg-[#fbf7ef] text-ink dark:bg-[#0e0c0a]">
      ...
      <div className="relative mx-auto grid min-h-[650px] max-w-[1440px] grid-cols-1 items-stretch px-6 lg:min-h-[720px] lg:grid-cols-[.78fr_1.22fr] lg:px-10 xl:px-14">
        <div className="relative z-10 flex flex-col justify-center py-16 lg:py-20">
          ...
          <h1 className="max-w-[8.2ch] font-display text-[58px] font-medium leading-[.92] tracking-[-.04em] text-ink sm:text-[72px] lg:text-[92px] xl:text-[104px]">Esencias que permanecen.</h1>
          <p className="mt-7 max-w-[440px] font-sans text-[15px] leading-7 text-muted lg:text-base">Aromia une perfumería, contexto editorial y descubrimiento para ayudarte a elegir una fragancia con criterio propio.</p>
          <div className="mt-9 flex flex-wrap items-center gap-6">
            <Link href="/catalogo" className="inline-flex min-h-12 items-center justify-center bg-gold-contrast px-7 font-plex text-[10px] uppercase tracking-[.17em] text-white transition hover:-translate-y-0.5 hover:shadow-lux focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-gold">Explorar colecciones</Link>
            <Link href="/quiz" className="nav-link font-sans text-sm text-ink">Descubrir mi perfil →</Link>
          </div>
          {perfume ? <div className="mt-10 flex max-w-[430px] items-center gap-3 border-t border-line pt-4 font-plex text-[9px] uppercase tracking-[.14em] text-muted"><span>Selección editorial</span><span className="h-px flex-1 bg-line"/><span>{perfume.marca}</span></div> : null}
        </div>
        ...
        <div className="relative min-h-[440px] lg:min-h-0">
          ...
          {perfume ? <div className="absolute bottom-7 left-[10%] right-[6%] z-[3] flex items-end justify-between gap-6 border-t border-line pt-4"><div><p className="font-display text-2xl font-medium leading-none text-ink lg:text-3xl">{perfume.nombre}</p><p className="mt-2 font-plex text-[9px] uppercase tracking-[.14em] text-muted">{perfume.marca}</p></div><span className="hidden font-plex text-[9px] uppercase tracking-[.14em] text-muted sm:inline">Objeto 01</span></div> : null}
        </div>
      </div>
    </section>
  );
