...
export function EditorialSelection({ perfumes }: { perfumes: Perfume[] }) {
  ...
  const family = publicText(feature.familia_olfativa);
  const concentration = publicText(feature.concentracion);
  const price = formattedReferencePrice(feature);
  const buyHref = `/api/catalog-buy/${encodeURIComponent(feature.slug)}`;
  ...
  return (
    <section className="mx-auto w-full max-w-[1440px] px-6 lg:px-10 xl:px-14">
      ...
      <div className="grid grid-cols-1 items-stretch gap-10 lg:grid-cols-[.72fr_1.08fr_.72fr] lg:gap-0">
        ...
        <a href={buyHref} target="_blank" rel="sponsored noopener noreferrer" aria-label={`Ver ${feature.nombre} en Amazon`} className="group relative min-h-[440px] overflow-hidden border-line bg-[#fbfaf7] lg:border-x dark:bg-[#f7f4ee]">
          <ProductImage slug={feature.slug} alt={`${feature.nombre} de ${feature.marca}`} mode="hero" />
          <span className="absolute bottom-5 right-5 border border-line bg-[#fffdf8]/90 px-3 py-2 font-plex text-[8px] uppercase tracking-[.14em] text-ink backdrop-blur">Amazon ↗</span>
        </a>
        ...
        <div className="flex flex-col justify-center py-4 lg:pl-12">
          <div className="grid grid-cols-2 gap-x-8 gap-y-7 border-b border-line pb-7">
            {family ? <div><p className="font-plex text-[9px] uppercase tracking-[.12em] text-muted">Familia olfativa</p><p className="mt-2 font-display text-lg text-ink">{family}</p></div> : <div />}
            {concentration ? <div><p className="font-plex text-[9px] uppercase tracking-[.12em] text-muted">Concentración</p><p className="mt-2 font-display text-lg text-ink">{concentration}</p></div> : <div />}
          </div>
          ...
          <div className="border-b border-line py-7">
            <div className="flex items-end justify-between gap-5">
              ...
              {feature.rating_promedio ? <div className="text-right"><p className="font-display text-3xl text-gold-contrast">{feature.rating_promedio.toFixed(1)}</p><p className="mt-1 font-plex text-[9px] uppercase tracking-[.12em] text-muted">Valoración</p></div> : null}
            </div>
          </div>
          ...
          <div className="border-b border-line py-7">
            ...
            <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-4">
              {notes.length > 0 ? notes.map((note, i) => <div key={note} className="flex items-center gap-3"><span className={`h-7 w-7 rounded-full border border-line ${i % 2 === 0 ? "bg-[#e9c46f]" : "bg-[#c6b497]"}`} aria-hidden="true"/><span className="font-sans text-xs text-ink">{note}</span></div>) : <span className="col-span-2 font-sans text-xs text-muted">Sin pirámide pública disponible.</span>}
            </div>
          </div>

          <div className="flex items-end justify-between gap-5 pt-7">
            <div><p className="font-plex text-[9px] uppercase tracking-[.12em] text-muted">Precio de referencia</p><p className="mt-2 font-display text-[28px] text-ink">{price ?? "Ver disponibilidad"}</p></div>
            <a href={buyHref} target="_blank" rel="sponsored noopener noreferrer" aria-label={`Comprar ${feature.nombre} en Amazon`} className="grid h-11 w-11 place-items-center rounded-full border border-line text-gold-contrast transition hover:border-gold hover:bg-soft">↗</a>
          </div>
        </div>
      </div>
    </section>
