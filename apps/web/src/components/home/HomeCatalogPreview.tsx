      ...
      <div className="mx-auto w-full max-w-[1440px] px-6 lg:px-10 xl:px-14">
        <div className="mb-7 flex flex-wrap items-end justify-between gap-5 border-b border-line pb-4">
          <div>
            ...
            <p className="mt-2 max-w-[54ch] font-sans text-xs leading-5 text-muted">Producto visual desde Amazon; lectura editorial y datos olfativos desde la base verificada de Aromia.</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
          {items.map((perfume) => {
            ...
            return (
              <article key={perfume.slug} className="group overflow-hidden border border-line bg-[#fffdf9] transition duration-300 hover:-translate-y-1 hover:border-gold hover:shadow-lux dark:bg-[#15110d]">
                <a href={buyHref} target="_blank" rel="sponsored noopener noreferrer" aria-label={`Ver ${perfume.nombre} en Amazon`} className="relative block aspect-[4/5] overflow-hidden bg-[#fbfaf7] dark:bg-[#f7f4ee]">
                  <ProductImage slug={perfume.slug} alt={`${perfume.nombre} de ${perfume.marca}`} mode="card" />
                  <span className="absolute right-3 top-3 z-[2] border border-line bg-[#fffdf9]/90 px-2 py-1 font-plex text-[7px] uppercase tracking-[.14em] text-muted backdrop-blur">Amazon ↗</span>
                </a>
                <Link href={`/catalogo/${perfume.slug}`} className="block p-4">
                  ...
                  {family ? <p className="mt-2 truncate font-plex text-[8px] uppercase tracking-[.1em] text-gold-contrast">{family}</p> : null}
                  <p className="mt-4 font-display text-lg text-ink">{price ?? "Ver disponibilidad"}</p>
