...
export function AromiaHome2026({ perfumes, articulos, categorias }: { perfumes: Perfume[]; articulos: Article[]; categorias: Categoria[] }) {
  ...
  return (
    <main className="overflow-hidden bg-[#f7f5f0] text-ink dark:bg-[#0e1311]">
      ...
      <section className="mx-auto w-full max-w-[1520px] px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
        <div className="grid gap-12 lg:grid-cols-[.62fr_1.38fr] lg:gap-24">
          <div className="lg:sticky lg:top-28 lg:self-start">
            <p className="font-plex text-xs uppercase tracking-[.15em] text-[var(--aromia-editorial-accent)]">Índice olfativo</p>
          ...
          <div className="border-t border-line">
            {index.map((category, indexNumber) => (
              <Link key={category.label} href={`/buscar?q=${encodeURIComponent(category.familias[0])}`} className="group grid min-h-[92px] grid-cols-[48px_1fr_auto] items-center gap-4 border-b border-line py-4 outline-none sm:grid-cols-[58px_1fr_1fr_auto] sm:gap-7">
                ...
                <span aria-hidden="true" className="font-display text-2xl text-[var(--aromia-editorial-accent)] transition group-hover:translate-x-1">→</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      {selection.length ? (
        <section className="mx-auto w-full max-w-[1520px] px-5 py-20 sm:px-8 lg:px-12 lg:py-28">
          <div className="mb-12 grid gap-8 lg:grid-cols-[1fr_.7fr] lg:items-end"><div><p className="font-plex text-xs uppercase tracking-[.14em] text-[var(--aromia-editorial-accent)]">Tres direcciones</p><h2 className="mt-4 max-w-[10ch] font-display text-[44px] leading-[.92] tracking-[-.04em] sm:text-[56px]">Distintas formas de ocupar el aire.</h2></div><p className="max-w-[42ch] font-sans text-base leading-7 text-muted lg:justify-self-end">Producto real, escala distinta, sin encerrarlo en una cuadrícula de tarjetas.</p></div>
      ...
      <section className="border-y border-line bg-[#e6e8e4]/45 dark:bg-[#151c18]"><div className="mx-auto grid max-w-[1520px] gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[.72fr_1.28fr] lg:px-12 lg:py-24"><div><p className="font-plex text-xs uppercase tracking-[.14em] text-[var(--aromia-editorial-accent)]">Magazine</p><h2 className="mt-4 max-w-[8ch] font-display text-[42px] leading-[.92] tracking-[-.04em] sm:text-[54px]">El perfume también se lee.</h2></div>{story ? <Link href={`/magazine/${story.slug}`} className="group block border-t border-line pt-5 outline-none"><p className="font-plex text-xs uppercase tracking-[.12em] text-muted">{story.categoria}</p><h3 className="mt-5 max-w-[18ch] font-display text-[38px] leading-[.96] tracking-[-.035em] text-ink transition group-hover:text-[var(--aromia-editorial-accent)] sm:text-[48px]">{story.titulo}</h3>{story.meta_description ? <p className="mt-6 max-w-[54ch] font-sans text-base leading-7 text-muted">{story.meta_description}</p> : null}<span className="mt-8 inline-flex min-h-11 items-center font-plex text-xs uppercase tracking-[.12em] text-[var(--aromia-editorial-accent)]">Leer historia →</span></Link> : <p className="font-sans text-muted">El archivo editorial está preparando su próxima historia.</p>}</div></section>

      <section className="mx-auto grid w-full max-w-[1520px] gap-10 px-5 py-20 sm:px-8 lg:grid-cols-[1fr_.85fr] lg:items-end lg:px-12 lg:py-28"><div><p className="font-plex text-xs uppercase tracking-[.14em] text-[var(--aromia-editorial-accent)]">Discovery</p><h2 className="mt-4 max-w-[10ch] font-display text-[44px] leading-[.92] tracking-[-.04em] sm:text-[58px]">Deja que el recorrido forme un mapa.</h2><p className="mt-6 max-w-[42ch] font-sans text-base leading-7 text-muted">Familias, notas y autorías se convierten en señales útiles a medida que exploras.</p><Link href="/descubrir" className="mt-8 inline-flex min-h-12 items-center border-b border-ink font-plex text-xs uppercase tracking-[.13em] text-ink">Abrir mi mapa <span aria-hidden="true" className="ml-4">↗</span></Link></div><div className="border-t border-line pt-6"><p className="font-display text-[30px] leading-tight tracking-[-.02em] text-ink">Menos cuestionario. Más observación.</p><p className="mt-4 max-w-[40ch] font-sans text-sm leading-6 text-muted">El sistema usa las dimensiones que ya existen en Aromia; no inventa porcentajes de compatibilidad.</p></div></section>
