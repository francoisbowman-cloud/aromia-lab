    ...
    <section className="grid min-h-[72vh] overflow-hidden border-y border-line bg-[#fffdf8] dark:bg-[#100d0a] lg:grid-cols-[1.1fr_.9fr]">
      <div className="relative min-h-[480px] overflow-hidden bg-[#e9dcc8] dark:bg-[#17120d] lg:min-h-[72vh]">
        ...
        <a
          ...
          aria-label={`Ver ${perfume.nombre} de ${perfume.marca} en Amazon`}
          className="absolute inset-[6%] overflow-hidden border border-black/[.06] bg-[#fbfaf7] shadow-[0_26px_65px_rgba(89,62,28,.10)] dark:border-white/10 dark:bg-[#f7f4ee] lg:inset-[7%_12%]"
        >
      ...
      <div className="flex flex-col justify-between px-6 py-10 lg:px-12 lg:py-14">
        <div>
          ...
          {publicText(perfume.descripcion_corta) ? <p className="mt-7 max-w-[46ch] font-sans text-[15px] leading-7 text-muted">{publicText(perfume.descripcion_corta)}</p> : null}
          ...
          <dl className="mt-10 grid grid-cols-2 border-y border-line font-sans text-sm">
            ...
            <div className="py-5 pl-5"><dt className="font-plex text-[8px] uppercase tracking-[.16em] text-muted">Concentración</dt><dd className="mt-2 text-ink">{concentration ?? "No especificada"}</dd></div>
            {family ? <div className="col-span-2 border-t border-line py-5"><dt className="font-plex text-[8px] uppercase tracking-[.16em] text-muted">Familia olfativa</dt><dd className="mt-2 font-display text-xl capitalize text-ink">{family}</dd></div> : null}
          </dl>
        </div>
        ...
        <div className="mt-12 flex items-end justify-between gap-6 border-t border-line pt-5">
          ...
          <a href={buyHref} target="_blank" rel="sponsored noopener noreferrer" className="border-b border-ink pb-1 font-plex text-[9px] uppercase tracking-[.14em] text-ink transition hover:border-gold hover:text-gold-contrast dark:border-[#f2ebdd]">Comprar en Amazon ↗</a>
        </div>
      </div>
    </section>
  );
