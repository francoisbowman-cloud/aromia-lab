      ...
      <div className="flex flex-col justify-between px-6 py-10 lg:px-12 lg:py-14">
        <div>
          ...
          <dl className="mt-10 grid grid-cols-2 gap-x-8 gap-y-7 font-sans text-sm">
            ...
            {perfumers.length ? <div className="col-span-2"><dt className="font-plex text-[8px] uppercase tracking-[.16em] text-muted">Perfumista{perfumers.length > 1 ? "s" : ""}</dt><dd className="mt-2 flex flex-wrap gap-x-4 gap-y-2">{perfumers.map((perfumer) => <Link key={perfumer.slug} href={`/perfumistas/${perfumer.slug}`} onClick={() => { recordPerfumerInterest(perfumer.slug, 3); trackEvent("perfumer_open", { perfumer_slug: perfumer.slug, source_perfume_slug: perfume.slug }); }} className="font-display text-xl text-ink transition hover:text-gold-contrast focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-gold">{perfumer.name}</Link>)}</dd></div> : null}
          </dl>
        </div>
