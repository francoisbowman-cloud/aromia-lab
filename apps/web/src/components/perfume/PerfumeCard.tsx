...
export function PerfumeCard({ perfume, variant = "catalog", index, trackingContext }: { perfume: Perfume; variant?: "catalog" | "featured"; index?: number; trackingContext?: "quiz_result" | "search" | "catalog" }) {
  ...
  return (
    <article className="group relative flex flex-col overflow-hidden border-b border-r border-line bg-[#fffdf8] transition-colors duration-500 hover:bg-[#f8f2e9] dark:bg-[#14100c] dark:hover:bg-[#18130f]">
      ...
      <Link href={`/catalogo/${perfume.slug}`} onClick={trackOpen} aria-label={`Abrir ficha de ${perfume.nombre} de ${perfume.marca}`} className="relative block aspect-[4/5] overflow-hidden bg-[#fbfaf7] outline-none dark:bg-[#f7f4ee]">
        <ProductImage slug={perfume.slug} imageUrl={perfume.imagen_url} alt={`${perfume.nombre} de ${perfume.marca}`} mode="card" />
        <span className="absolute bottom-4 right-4 grid h-9 w-9 place-items-center border border-[rgba(33,29,23,.2)] bg-[rgba(251,248,243,.88)] font-display text-lg text-ink backdrop-blur-sm transition-transform group-hover:translate-x-1">→</span>
      </Link>
