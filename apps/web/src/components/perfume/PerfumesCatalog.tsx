  ...
  return (
    <div>
      <div className="sticky top-16 z-20 border-y border-line bg-[rgba(251,248,243,.96)] backdrop-blur-md dark:bg-[rgba(15,12,9,.96)]">
        <div className="flex items-center gap-4 border-b border-line px-0 py-3 font-plex text-[8px] uppercase tracking-[.18em] text-muted">
          <span>Refinar selección</span><span className="h-px flex-1 bg-line"/><span>{activeCount === 0 ? "Archivo completo" : `${activeCount} filtros activos`}</span>
        </div>
      ...
      {filtrados.length === 0 ? (
        <div className="grid min-h-[360px] place-items-center border-b border-line py-16 text-center"><div><p className="font-plex text-[9px] uppercase tracking-[.2em] text-gold-contrast">Sin coincidencias</p><p className="mt-4 font-display text-4xl text-ink">Amplía el mapa olfativo.</p><p className="mx-auto mt-4 max-w-[38ch] font-sans text-sm leading-6 text-muted">La combinación actual es demasiado específica. Retira una condición o vuelve al archivo completo.</p><button type="button" onClick={clearFilters} className="mt-7 border-b border-ink pb-1 font-plex text-[10px] uppercase tracking-[.14em] text-ink transition hover:border-gold hover:text-gold-contrast">Ver colección completa</button></div></div>
      ) : (
        <div className="grid grid-cols-1 border-b border-line sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filtrados.map((perfume, index) => <PerfumeCard key={perfume.slug} perfume={perfume} index={index} />)}</div>
      )}
    </div>
  );
}
