...
export function NewsletterForm({ fuente, mensajeExito = "Listo — te avisamos cuando haya bajadas de precio y contenido nuevo." }: { fuente: "home" | "quiz" | "club"; mensajeExito?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-1 border-y border-line sm:grid-cols-[1fr_auto]">
      <label className="absolute -left-[10000px] top-auto" htmlFor={`newsletter-${fuente}`}>Correo electrónico</label>
      <input id={`newsletter-${fuente}`} type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="tu@email.com" className="min-h-14 border-0 border-b border-line bg-transparent px-1 font-sans text-sm text-ink outline-none placeholder:text-muted focus:bg-soft sm:border-b-0 sm:border-r sm:px-4" />
