...
export function personalizedPerfumeScore(perfume: Perfume, profile: DiscoveryProfile, source?: Perfume | null): PersonalizedPerfumeResult {
  ...
  if (familyWeight > 0) { score += Math.min(36, familyWeight * 4); reasons.push(`afinidad con ${perfume.familia_olfativa}`); }
  ...
  if (noteWeights.length) { score += Math.min(28, noteWeights.slice(0, 4).reduce((sum, item) => sum + item.weight * 2, 0)); reasons.push(`notas afines: ${noteWeights.slice(0, 2).map((item) => item.note).join(" + ")}`); }

  const perfumerWeights = perfumersForPerfume(perfume.slug)
    .map((perfumer) => ({ perfumer, weight: profile.perfumers[perfumer.slug] ?? 0 }))
    .filter((item) => item.weight > 0)
    .sort((a, b) => b.weight - a.weight);
  if (perfumerWeights.length) { score += Math.min(18, perfumerWeights[0].weight * 3); reasons.push(`obra de ${perfumerWeights[0].perfumer.name}`); }
