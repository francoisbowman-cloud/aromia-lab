-- Amazon commerce/image policy for every active published perfume.
-- Fragrantica and other editorial sources may remain in source_url for olfactory provenance,
-- but they are not valid public image/commerce sources.

UPDATE perfumes
SET amazon_url = link_afiliado,
    actualizado_en = now()
WHERE activo = true
...
UPDATE perfumes
SET link_afiliado = amazon_url,
    actualizado_en = now()
WHERE activo = true
  AND estado = 'publicado'
  AND (
    link_afiliado IS NULL
    OR btrim(link_afiliado) = ''
    OR lower(btrim(link_afiliado)) = 'pending'
    OR link_afiliado !~* '^https?://([^/]*\.)?amazon\.com/'
  )
...
BEGIN
  ...
  WHERE activo = true
    AND estado = 'publicado'
    AND NOT (
      COALESCE(NULLIF(btrim(link_afiliado), ''), NULLIF(btrim(amazon_url), ''))
      ~* '^https?://([^/]*\.)?amazon\.com/'
    );
