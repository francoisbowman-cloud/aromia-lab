...
function normalizePriceSegment(value: unknown): SourceRow["priceSegment"] {
  ...
  if (!VALID_PRICE_SEGMENTS.has(normalized)) throw new Error(`invalid price_segment ${JSON.stringify(value)}`);
  return normalized as SourceRow["priceSegment"];
}
...
function loadBatch(): { rows: SourceRow[]; slugs: string[]; sha256: string } {
  ...
  if (sha256 !== ACCEPTED_ARTIFACT_SHA256) {
    throw new Error(`batch-003 artifact hash mismatch: expected ${ACCEPTED_ARTIFACT_SHA256}, got ${sha256}`);
  }
...
async function insertRow(client: PoolClient, row: SourceRow) {
  await client.query(
    ...
    [
      ...
      row.dataConfidence, BATCH_SOURCE, row.subfamily, row.launchYear,
...
async function importBatch(client: PoolClient, rows: SourceRow[], slugs: string[], sha256: string) {
  ...
  if (afterTotal !== beforeTotal + EXPECTED_ROWS || afterPublished !== beforePublished + EXPECTED_ROWS || imported.length !== EXPECTED_ROWS) {
...
async function rollbackBatch(client: PoolClient, slugs: string[], sha256: string) {
  ...
  if (rows.length !== EXPECTED_ROWS || rows.some((row) => row.catalog_source !== BATCH_SOURCE)) {
    throw new Error(`rollback preflight refused: expected exactly ${EXPECTED_ROWS} rows owned by ${BATCH_SOURCE}, got ${rows.length}`);
  }
  await client.query("BEGIN");
  try {
    const deleted = await client.query("DELETE FROM perfumes WHERE catalog_source=$1 AND slug = ANY($2::text[])", [BATCH_SOURCE, slugs]);
    if (deleted.rowCount !== EXPECTED_ROWS) throw new Error(`rollback expected ${EXPECTED_ROWS} deletes, got ${deleted.rowCount}`);
    await client.query("COMMIT");
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
...
async function main() {
  ...
  const { rows, slugs, sha256 } = loadBatch();
  const action = process.env.CATALOG_IMPORT_ACTION ?? "import";
  if (!new Set(["import", "rollback"]).has(action)) throw new Error(`unsupported CATALOG_IMPORT_ACTION=${action}`);

  const pool = new Pool({ connectionString: process.env.DATABASE_URL });
  const client = await pool.connect();
  try {
    if (action === "rollback") await rollbackBatch(client, slugs, sha256);
    else await importBatch(client, rows, slugs, sha256);
  } finally {
    client.release();
    await pool.end();
  }
...
main().catch((error) => {
  console.error("CATALOG_BATCH_003_FAILED", error);
  process.exit(1);
});
