...
async function updateRow(client: PoolClient, row: RawRow) {
  ...
  const commercialUrl = clean(row.amazon_url);
  ...
  const result = await client.query(
    `UPDATE perfumes SET
      ...
      link_afiliado = COALESCE(NULLIF(link_afiliado, ''), $16),
    ...
    [
      ...
      clean(row.image_url), clean(row.image_source), commercialUrl, clean(row.affiliate_status), clean(row.visual_quality),
...
async function coverage(client: PoolClient, slugs: string[]) {
  return (await client.query(`
    SELECT
      ...
      COUNT(*) FILTER (WHERE slug = ANY($1::text[]) AND link_afiliado IS NOT NULL AND link_afiliado <> '')::int AS frontend_affiliate_links,
...
async function main() {
  ...
  if (rows.some((r) => !clean(r.image_url) || !clean(r.amazon_url) || !clean(r.description) || !clean(r.seo_title) || !clean(r.seo_description))) {
    throw new Error("published enrichment is not publication-complete");
  }
  ...
  try {
    ...
    if (Number(after.images) !== EXPECTED_ROWS) throw new Error(`production image coverage is ${after.images}/${EXPECTED_ROWS}`);
    if (Number(after.amazon_links) !== EXPECTED_ROWS) throw new Error(`production Amazon link coverage is ${after.amazon_links}/${EXPECTED_ROWS}`);
    if (Number(after.frontend_affiliate_links) !== EXPECTED_ROWS) throw new Error(`frontend link coverage is ${after.frontend_affiliate_links}/${EXPECTED_ROWS}`);
    if (Number(after.descriptions) !== EXPECTED_ROWS) throw new Error(`description coverage is ${after.descriptions}/${EXPECTED_ROWS}`);
    if (Number(after.seo_titles) !== EXPECTED_ROWS || Number(after.seo_descriptions) !== EXPECTED_ROWS) throw new Error("SEO coverage is incomplete");
