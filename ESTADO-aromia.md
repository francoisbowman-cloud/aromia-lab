...
Última actualización: 5 de agosto de 2026 — por: Code (Claude Code). **Fase 1 del sistema de imágenes (fundación técnica y documental) cerrada** — PR #4 fusionado a `main` (squash, commit `80a6d05`), sin procesar ni tocar ninguna imagen de producción — decisión #98. Actualización previa, mismo día: `main` pasó a ser la única rama del repo y la única fuente de verdad, consolidando el contenido real de producción (antes en `feature/v2.0`) — decisión #97.
...
**Fase 2 del sistema de imágenes — piloto visual de 5 perfumes: CERRADA
el 2026-08-06** (decisión #98 → #99). Aventus, Baccarat Rouge 540,
Sauvage EDT (renombrado, ver decisión #99), Black Opium EDP, Erba Pura.
Resultado consolidado en `reports/image-audits/consolidated-pilot.{json,md}`
y `treatment-plan.{json,md}`. `scripts/images/optimize.mjs` nunca se
invocó — el piloto nunca llegó a necesitar procesamiento binario porque
los defectos reales resultaron ser de identidad/nombre (Sauvage) o
quedaron diferidos sin bloquear (Aventus, Black Opium), no de recorte o
fondo. Próximo paso de imágenes: decidir si se escala el mismo método de
auditoría (ChatGPT + Cowork + verificación humana) a los 50 perfumes del
catálogo, o si se prioriza otro bloque del roadmap primero — sin decidir
todavía, no arranca sin luz verde explícita de Brey.
...
3. Retomar la verificación de **Lote 01 (6 candidatos sin precio USD)** y **Lote 02 (20 candidatos aún sin verificar, D&G/Armani)** — necesita otra sesión con navegador, idealmente sin la restricción de región a República Dominicana que bloqueó buena parte de esta ronda.
...
7. **Pautado por Brey, sin fecha (decisión #93)**: generar recortes sin fondo de los 50 perfumes vía OMNI y usarlos como insumo creativo en un rediseño de secciones de Aromia (Design + OMNI) — explícitamente para una sesión futura, no la próxima acción inmediata.
