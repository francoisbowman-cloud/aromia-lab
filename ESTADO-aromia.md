...
— fix de fondo del defecto de opacidad sobre tokens `var()` de Tailwind
(PR #163, mergeado). No se convirtió el token base (`--text` etc. siguen hex,
todo consumidor `var(--text)` crudo sigue igual): cada token con consumidores
de opacidad (`paper`/`surface`/`ink`/`muted`/`soft`/`gold`/`destructive`) ganó
un compañero `--x-rgb` (canales espaciados) junto a su hex, en el CSS que gana
el cascade; `tailwind.config.ts` apunta esos colores a
`rgb(var(--x-rgb) / <alpha-value>)`. `line` no se tocó (translúcido a
propósito). Los parches de PR #157/#160 en superficie pública se revirtieron a
sintaxis limpia; los de `admin/` quedan igual (fuera de alcance). QA en rama
antes de mergear: `tsc`/`lint` limpios, compilación standalone de Tailwind
...
**Actualización 2026-09-12 (decisión #110):** **piloto OMNI cerrado, D-11
resuelto.** El MCP `image-toolkit` se conectó en esta sesión y se corrió el
pipeline completo (`remove_background` → `autotrim_image` → `resize_image` →
`convert_image`) para los 10 slugs de `docs/images/OMNI-PILOT-2026-09-11.md`,
clasificados A/B/C/D según el handoff §5 — detalle completo en
`docs/images/OMNI-PILOT-RESULTADOS-2026-09-12.md`. Resultado: **7 en A**
(`bleu-de-chanel-edp`, `chanel-no5-edp`, `santal-33-edp`, `aventus`,
`molecule-01`, `acqua-di-gio-edt`, `nishane-hacivat`) registrados en
`PERFUME_CUTOUTS` con sign-off de Brey; **1 en B** (`oud-wood`, artefacto de
borde en la unión tapa/cuello) queda fuera del lote aprobado; **2 en C**
(`baccarat-rouge-540-edp`, `1-million`, foto con caja) **se quedan con su
caja a pedido explícito de Brey** — no se re-sourcea ni se genera imagen IA
por ahora, pausado sin fecha. Colateral resuelto: la fuente de
`nishane-hacivat` (`cdn0.woolworths.media`) devolvía 403 — se re-sourceó vía
Amazon (`B07TTCL5HY`, confirmado por Brey con el link del listing real) y se
actualizó `imagen_url` en la Postgres de producción vía
`PATCH /api/admin/perfumes/44` (mismo mecanismo administrativo de la
decisión #106). Nota técnica para escalar a los 125: el base64 de
`get_result_base64` truncó sin aviso dos veces en payloads medianos
(~15-20 KB) — se resolvió bajando el tamaño de salida del pipeline
(width=300 en vez de 620), causa raíz no identificada. Trazabilidad
SHA-256 por asset (handoff §8) sigue sin implementarse — pendiente antes de
...
| 104 | **Housekeeping del ticket de transición ejecutado (bloque 1.1) — 2026-08-27, Code**. (a) **Ramas**: de 41 ramas sueltas además de `main` se borraron 26 en `origin` con aprobación textual de Brey — todos los clusters `omni/*` (11) y `agent/*` (9) de experimentos de OMNI, más la línea catalog-scale (`feat/catalog-expansion-automation-v1-batch-003`, `ops/catalog-import-batches-001-002`, `ci/catalog-expansion-v1-gate`) tras verificar que **ningún dato de perfumes se perdía** — `catalog/imports/batch-001/002/003.csv` y `apps/api/src/db/importCatalogBatch003.ts` ya están en `main`, y la copia de `batch-003.csv` en `main` está más enriquecida que la de las ramas. En una segunda pasada, con un nuevo "Kill" textual de Brey, se borraron las **13 restantes** (experimentos visuales/CI de OMNI de agosto + 3 de Brey: `feat/aromia-visual-conversion-upgrade`, `remove/ai-generated-editorial-images`, `docs/session-state-2026-08-07`) tras verificar que la única migración que aportaban — `015_amazon_catalog_commerce_gate.sql`, política de URLs Amazon, sin datos de perfumes insertados — ya está en `main` (junto con las 016-020). **Total: 39 de 41 ramas sueltas borradas.** Quedan solo 2 ramas no-`main`, ambas conservadas a propósito: `feat/catalog-pipeline-500` (PR #10, decisión #101, worktree propio) y `feat/aromia-editorial-motion-skill` (worktree activo) — más las ramas efímeras de los PR abiertos de esta sesión (#113, #114, #115). El repo vuelve a la regla de "una sola rama + temporales de PR". (b) **Railway** (`aromia-lab-v2`): las 10 preview huérfanas/stale (`web-aromia-redesign-preview`, `-preview-v2`, `web-omni-home-fire-test`, `web-taste-preview-01`, `web-mockup-fidelity-final`, `catalog-expansion-b003-validation`, `web-hybrid-signature-01`, `web-visual-preview-02`, `catalog-import-b001-b002`, `web-visual-preview`) quedaron **marcadas para borrado (staged)** vía el agente de Railway con aprobación de Brey — el commit final (`accept-deploy` sobre el entorno `production`, que además redeploya `web`/`api`) fue **bloqueado por el clasificador de seguridad de esta sesión**; falta que Brey aplique los cambios staged desde el dashboard de Railway. `web`/`api`/`Redis`/`Postgres-TdTp` sin tocar. (c) **Token `--bg` duplicado consolidado**: eliminada la definición muerta de `globals.css` (`#fbf8f3`/`#0e0c0a`), `--bg` queda solo en `aromia-redesign.css` (importado después en `layout.tsx`). Se plegó en el mismo commit, como WIP intencional aprobado por Brey, el trabajo no commiteado de OMNI que además adelanta el bloque 1.3 (lienzo blanco) por la **Opción A** del `ticket-lienzo-blanco-flotante.md`: `--bg`/`--background` a `#FFFFFF` / `#0A0A0A` puro, y `ProductImage.tsx` aplica la máscara radial + `mix-blend-multiply` en todos los modos (prop `surface` y tipo `Surface` eliminados, `bg-white` hardcodeado quitado de `PerfumeCard`, call sites de Home/Discovery/HeroHeader/HomeHero ajustados). `tsc --noEmit` y `next lint` limpios en `apps/web` | Brey (aprobación de cada acción) / Code (ejecución) |
| 105 | **Aromia pasa a revista editorial visualmente ambiciosa — recortes sin fondo + IA autorizada para los casos difíciles (Brey, 2026-08-29)**. (a) **Se activa la decisión #93**: cada perfume del catálogo (hoy **125**, no 50) se muestra como un **recorte con canal alfa** (PNG sin fondo) generado con `remove_background` de OMNI (MCP conectado, infra propia en Railway, sin costo por operación reportado) sobre su `imagen_url`. Almacenamiento según #93: `apps/web/src/../public/perfumes/cutouts/<slug>.png` versionado + fallback en código, sin migración de DB. (b) **`ProductImage.tsx` pasa a "cutout-first"**: si existe el recorte local → se muestra directo con `drop-shadow()` sutil, sin `mix-blend-multiply` ni máscara radial; si no → tratamiento actual (Opción A) como fallback. Esto **invierte la Opción A→B** del `ticket-lienzo-blanco-flotante.md` y resuelve de paso el problema de modo oscuro del bloque 1.3 (un PNG con alfa flota igual sobre `#FFFFFF` y sobre `#0A0A0A`). (c) **Invierte la política anti-IA** de las decisiones #67 y #96 y de la ex-rama `remove/ai-generated-editorial-images`: para los frascos donde el recorte automático no sirve (vidrio transparente, líquidos de color, tapas cromadas) o la foto trae packaging no deseado, se **sustituye por imagen generada por IA de alto nivel de detalle**, con la **regla dura de NO inventar ningún detalle del frasco** — fidelidad 1:1 a la identidad real (forma, color, tapa, grabados, etiqueta, proporción). **Las genera ChatGPT** (capacidad de imagen incluida en su plan), según la tabla de responsabilidades del manual operativo ("Generación de imágenes | ChatGPT | Code para publicación") — Code NO genera imágenes: arma la lista de slugs difíciles con el motivo, ChatGPT produce, Code integra el asset y registra el slug. (d) **Criterio de Code (Brey delegó ejecución):** las fotos con **caja** se tratan como caso difícil → recorte solo del frasco o IA (el packaging es lenguaje de retail, no de revista); **piloto de ~8-10 perfumes primero** cubriendo los tipos de frasco duros, con deploy de preview para que ChatGPT + Brey lo validen antes de procesar los 125; Opción A (máscara CSS) se conserva solo como fallback técnico. (e) **De acá en más el trabajo editorial es intensivo en composición visual** — muchas composiciones editoriales con estos recortes; dirección con los skills `aromia-image-art-direction` / `perfume-art-director` / `magazine-layout-designer`. Se actualiza el comentario de política de `ProductImage.tsx` y la nota del `omni-strict-audit` (el recorte es **offline**; el componente solo consume el PNG, así que el guard `destructive_background_extraction` sigue válido) | Brey (decisión de producto) / Code (asigna número, ejecuta piloto) |
| 106 | **Reference Issue 01 desbloqueada + barrido de bugs visuales de ficha/magazine + ticket maestro de Diseño (2026-09-08/10, Code).** 5 PR (#156-#160), todos mergeados, desplegados y verificados en producción. **(a) Reference Issue 01 ("Aromia · Número 01")** — arrancó desde un `.zip` de Chat con dos docs rectores incorporados a la raíz: `PLAN-Aromia-Reference-Issue-01.md` (dirección de Diseño, 16 piezas / 3 actos) y `ticket-arquitectura-editorial-tiptap.md` (arquitectura para Code: modelo híbrido `contenido_html` + `contenido_json` JSONB nuevo, 5 primitives Level B, sanitización server-side, test suite de 10 casos — **Fase 2, gateada, sin arrancar**). Code auditó el ticket contra el código real: correcto. Cerró el bloqueante PLAN §B en `INVENTARIO-CONTENIDO-RI01.md` con datos de producción (15 artículos publicados, catálogo de 125, sin ficha de perfumista, `drafts/` con ~46 piezas `editorial_ready` de Cowork). Decisiones de Brey: nombre "Aromia · Número 01"; perfumista de las piezas 11-12 = **Jean-Claude Ellena** (perfil solo sobre obra publicada + entrevistas on-record); se encarga UN ensayo largo nuevo para el Acto II (BR540 hype vs. mérito); bylines = "Redacción Aromia". Los 4 encargos cerrados: **C** = `autor = "Redacción Aromia"` en los 15 artículos publicados (estaban NULL) vía `PATCH /api/admin/articles/:id` — escritura real a producción, verificada 15/15 contra la API pública (el `--apply` masivo lo corrió Brey; el clasificador de seguridad bloqueó a Code, igual que en la decisión #91). **A/B/D** = 3 borradores de Cowork ingestados a `drafts/` (`br540-hype-vs-merito.md`, `jean-claude-ellena-la-sustraccion.md`, `matriz-olfativa-numero-01.md`); Code validó la matriz D contra Postgres (familia/género 15/15). Mergeado en **PR #156**. Hallazgo abierto (no se toca el catálogo desde este flujo): `baccarat-rouge-540-edp` tiene `notas_fondo` = cedro/abeto (ausentes de la ficha oficial de Maison Francis Kurkdjian), `precio_referencia` 325 vs. 360 USD retail sep-2026, `anio` y `perfumista` en NULL. **(b) Ticket maestro de Diseño** (`ticket-diseno-aromia-punta-a-punta.md`, raíz) — consolida todo el alcance de Diseño en 5 workstreams: WS-1 deuda visual en prod, WS-2 RI01 Fase 1 (revista pura, puede arrancar), WS-3 RI01 Fase 2 / Tiptap (gateada), WS-4 escala tipográfica `--type-*` + fix de fondo de tokens de opacidad, WS-5 extracción del sistema (post-aprobación). 10 decisiones abiertas de Brey (D-1…D-10). **(c) Defecto de opacidad sobre tokens `var()` — PR #157 + #160.** Los colores de `tailwind.config.ts` son `var(--x)` pelado sin slot `<alpha-value>`, así que Tailwind v3.4 **no genera** `.bg-gold\/15`, `.text-ink\/[.028]`, `.bg-soft\/30`, etc. — el elemento hereda del ancestro (texto = casi-negro del body; fondo = transparente). Síntoma que lo destapó: el wordmark fantasma gigante de `HeroHeader.tsx` (`text-ink/[.028]`) renderizaba negro sólido y desbordado en la ficha `/catalogo/[slug]`. **PR #157**: 5 sitios (`HeroHeader`, `PerfumeCard`, `MagazineCategoryNav` ×2, `descubrir/familias`) → `opacity-*` / `bg-white/[0.88] dark:bg-[#0a0a0a]/[0.88]`; **más**: la casilla de reseñas vacía de la ficha (`04 / Comunidad` con "— / 0 reseñas") ahora solo se monta si `perfume.rating_promedio` tiene valor. **PR #160**: ~13 sitios restantes (`soft`/`surface` → token entero; `gold`/`destructive` → `bg-[color:color-mix(in_srgb,var(--gold)_15%,transparent)]`, patrón ya usado en `subBatch01Story.module.css`; `text-muted/40` → `text-muted opacity-40`; skeleton `bg-line/40` → `bg-line`). El `grep` de opacidad-sobre-token queda en cero en `apps/web/src`. **El fix de fondo** (migrar los tokens a formato de canales, `--text: 32 35 31` + `rgb(var(--text) / <alpha-value>)`, toca 4 CSS y todo consumidor de `var(--text)`) sigue pendiente = **WS-4**, con QA en claro+oscuro. **(d) `perfumeSlugs` desactualizados en `lib/perfumers.ts` — PR #158.** Varios slugs no coincidían con el catálogo publicado → `perfumersForPerfume()` no resolvía la atribución en la ficha y `/perfumistas` subcontaba "obras". 4 remapeos (`baccarat-rouge-540`→`-edp`, `portrait-of-a-lady`→`-edp`, `light-blue-edt`→`light-blue`, `black-afgano`→`black-afgano-extrait`) + 13 slugs muertos eliminados (perfumes fuera del catálogo; el comentario del archivo pide re-agregarlos cuando entren). Efecto: las fichas de BR540, Black Afgano, Portrait of a Lady y Light Blue ahora **sí muestran su perfumista** (antes no aparecía ninguno). NO resuelve el mínimo de 5 obras por perfumista (cuentan 1-4) — expansión de contenido, decisión aparte; además solo 2 de 13 perfumistas tienen `portrait`. **(e) Héroe de artículo blanco/blanco + lector page-flip — PR #159, verificado en vivo.** `ArticleHero` pintaba el degradé de respaldo (artículos sin `imagen_portada_url` — los 15) en un hijo `absolute inset-0 -z-10`; la `<section>` padre, sin `z-index` ni stacking context propio, lo mandaba **detrás de `<main>`** (opaco) → el `<h1 text-white>` quedaba blanco sobre crema (~1,05:1). Fix de raíz: el fondo va como `background` de la propia `<section>`, con `bg-[#15110d]` de piso sólido. El lector `PageFlipReader` era ancho-driven + `overflow-hidden` + `autoSize` → el libro se clavaba arriba y recortaba el texto, sin aire para la animación → contenedor pasado a **alto-driven** (`h-[min(90dvh,880px)]`, ancho de una página portrait), centrado, `autoSize={false}`, `maxShadowOpacity` 0.3→0.5, `flippingTime` 650→700, `CHARS_PER_PAGE` 550→460. Verificado en `aromialab.com/magazine/guia-perfumes-verano`: titular legible sobre el degradé, lector centrado, texto completo por página, hojeo funciona. **(f) Higiene:** `A.txt` (Brey dejó el `ADMIN_API_TOKEN` en la raíz del checkout para el encargo C) borrado y agregado a `.gitignore` junto con `*.secret.txt` | Brey (decisiones de producto, `--apply` del encargo C, merges) / Code (auditoría, PRs, verificación) / Cowork (borradores A/B/D) / Chat (docs rectores de RI01) |
...
Se borraron en dos pasadas, cada una con "Kill" textual de Brey y tras
verificar que no se perdía dato de perfumes: primero 26 (clusters
`omni/*`, `agent/*`, línea catalog-scale), después las 13 restantes
(experimentos visuales/CI de agosto + 3 de Brey). **39 de 41 borradas.**
Quedan solo **2 ramas no-`main`, ambas a propósito**:
...
(worktree activo). Más las ramas temporales de los PR abiertos de la
sesión de transición (#113 borrador editorial, #114 bloque 1.1, #115
bloque 1.2). El repo vuelve a "una sola rama + temporales de PR".
...
- **Reference Issue 01 (decisión #106), todos en la raíz:** `PLAN-Aromia-Reference-Issue-01.md` (dirección de Diseño), `ticket-arquitectura-editorial-tiptap.md` (arquitectura editorial Tiptap, Fase 2 gateada), `INVENTARIO-CONTENIDO-RI01.md` (inventario de contenido + review de Code de los encargos), `brief-cowork-RI01-encargos.md` (encargos A/B/D para Cowork), `ticket-diseno-aromia-punta-a-punta.md` (ticket maestro de Diseño, 5 workstreams). Borradores de Cowork para el número: `drafts/br540-hype-vs-merito.md`, `drafts/jean-claude-ellena-la-sustraccion.md`, `drafts/matriz-olfativa-numero-01.md`.
...
- **Reseñas de comunidad real** (decisión #78): `CommunityReviews.tsx` usa rating + texto cargados a mano por el admin; el ticket original pide datos reales de Amazon. Necesita que Brey confirme si el enfoque actual alcanza o si hace falta scraping real de reseñas. **Nota 2026-09 (decisión #106, PR #157):** la casilla vacía ("— / 0 reseñas") ya no se muestra — la sección `04 / Comunidad` de la ficha solo se monta si hay `rating_promedio`. Falta el plan de datos reales.
...
**Reference Issue 01 + Diseño (decisión #106), abierto:**
- **Design arranca WS-2 (RI01 Fase 1)** con OK textual de Brey (D-8) — el contenido ya está: los 3 borradores de Cowork en `drafts/` + las ~46 piezas editoriales previas. Los 3 borradores A/B/D siguen sin publicar en el Magazine (publicación manual de Brey vía `/admin/magazine`, o script de importación a evaluar).
- **10 decisiones de Brey (D-1…D-10)** en `ticket-diseno-aromia-punta-a-punta.md`: fuente de datos de `rendimiento en piel` (D-2, `longevidad`/`estela`/`proyeccion` NULL en todo el catálogo); fotos de los ~11 perfumistas sin `portrait` y gate de ≥5 obras (D-3/D-4); tratamiento dark de `ProductImage` (D-5); historias SubBatch 01 que devuelven 500 en prod (D-6, pipeline editorial); sign-off de dirección de arte para la escala tipográfica `--type-*` y el hero de Omán (D-7); gates de arranque de WS-2 y WS-3 (D-8/D-9); dirección del Quiz en Discovery (D-10).
- **WS-4 — fix de fondo de los tokens de opacidad**: migrar los tokens de color de `tailwind.config.ts` a formato de canales para que los modificadores `/N` funcionen sin `color-mix` arbitrario. Toca los 4 CSS de tokens y todo consumidor de `var(--text)`; necesita QA en claro+oscuro. Los ~18 sitios rotos ya están parchados (PRs #157/#160), esto es la corrección estructural.
- **RI01 Fase 2 (`ticket-arquitectura-editorial-tiptap.md`)** — modelo híbrido `contenido_json` JSONB + 5 primitives Tiptap + sanitización server-side + test suite. Gateada: después de la auditoría intermedia de Design (WS-2) y OK de Brey. La sanitización server-side de `contenido_html` (hoy inexistente, hallazgo del ticket) entra con esta fase.
- **Dato de catálogo `baccarat-rouge-540-edp`**: `notas_fondo` cedro/abeto ausentes de la ficha oficial MFK; `precio_referencia` 325 vs. 360 USD; `anio` y `perfumista` en NULL. Revisión de datos aparte — no se toca el catálogo desde el flujo editorial.
- **Citas de *The Diary of a Nose*** en `drafts/jean-claude-ellena-la-sustraccion.md`: son de 2ª mano (vía reseña de Grain de Musc), marcadas como tales; verificar contra el libro original si se publica.
