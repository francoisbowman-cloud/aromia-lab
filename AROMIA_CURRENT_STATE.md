...
LAST_ACTOR: Code — Discovery/Search bounded-query implementation
LAST_ACTION: Executed `handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md` (Publisher-designated focus) end-to-end on a dedicated branch. Added `GET /api/perfumes/search` (ILIKE on nombre/marca/familia/notas, pagination), `/facets` (family aggregates) and `/discovery-seed` (bounded diverse sample, ≤8/family) — all returning a new compact `DiscoveryPerfume` DTO (`apps/web/src/lib/types.ts`) instead of full rows; `GET /api/perfumes`/`:slug` untouched. Migration `021_discovery_search_indexes.sql` adds `pg_trgm` GIN indexes on nombre/marca plus btree on familia_olfativa and (activo, estado, nombre). `/buscar` (`apps/web/src/app/buscar/page.tsx`, `DiscoverySearch.tsx`) ships zero perfume rows on an empty query, resolves a present query server-side on first paint, then debounces (300ms)/cancels (AbortController, 6s timeout) client-side re-queries via new `apps/web/src/lib/searchClient.ts` (calls the public API URL directly — the private Railway-internal base only resolves server-side) and re-ranks only the bounded candidate set with the existing `discoveryTextScore`/`personalizedPerfumeScore` (unchanged algorithm, unchanged `internal_search`/`discovery_product_clicked`/`search_content_clicked` analytics names+params). `/descubrir` (`DiscoveryDashboard.tsx`) now derives the family atlas from `/facets` and ranks the 6 recommendations + atlas preview over the bounded `/discovery-seed` sample instead of the full catalog — a deliberate, documented trade-off the handoff explicitly allows ("bounded recommendation candidate set"), not exhaustive ranking. Privacy contract unchanged: olfactory profile stays `localStorage`-only, never sent to the server; discovery-seed/facets endpoints take no personal signal as input. `discovery.ts`/`personalization.ts`/`PerfumeCard.tsx` retyped from `Perfume` to `DiscoveryPerfume` (a strict `Pick<Perfume,...>` subset) so all 9 other `getPerfumes()` callers keep compiling unchanged. Evidence (local seeded catalog, same environment, indicative of scaling not exact production bytes): full `/api/perfumes` 58,038 B (≈1,161 B/row) vs. bounded `/search`/`discovery-seed` responses in the 7–24 KB range regardless of catalog size — `/buscar` with no query now ships 0 perfume bytes. Verified end-to-end against a local API + seeded Postgres in the Browser pane: search-as-you-type (debounce, cancellation, re-rank, 12 results), empty/loading/error states (a real CORS rejection rendered the recoverable-error copy, not a crash), and the atlas/recommendations flow all render correctly with no console errors; graceful degradation confirmed separately (facets/seed unavailable → atlas falls back to "Todo" only, no crash). `tsc --noEmit`, `next lint` and `npm run build` (49/49 routes) clean on `apps/web`; `tsc -p tsconfig.json` clean on `apps/api`. Landed as [PR #178](https://github.com/francoisbowman-cloud/aromia-lab/pull/178) — **OPEN, not merged, not deployed.** OMNI desktop/mobile regression QA (handoff acceptance criteria) not run in this session — needed before merge. Does not touch and does not silently resolve `SUBBATCH_01_RUNTIME_P0`, `MAGAZINE_ARTICLE_HERO_CONTRAST_P1`, WS-6 Design work, cutout QA/traceability, or the Tiptap/block-editor redesign (explicit non-goal) — all remain open exactly as below.
PRIOR_LAST_ACTOR_CONTEXT_V53: ChatGPT / Sites evaluation — audited whether Aromia should adopt Sites and converted the Publisher's selected priority into a durable Code handoff. Evaluated the live Aromia surfaces and current repository architecture. Decision: keep GitHub + Next.js + Express/Postgres/Redis + Railway + OMNI as the canonical stack; do not duplicate or migrate the magazine, catalog, Discovery, search or CMS into Sites. The Publisher explicitly selected point 3 as the next focus: Discovery and search must stop transporting the complete perfume catalog to the browser. Added `handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md` with current-code evidence, phased implementation, privacy constraints, acceptance criteria and QA/PR boundaries. No application code or production state changed in that turn.
PRIOR_LAST_ACTOR_CONTEXT_V52: Code — translated `brief-design-ws6-cutouts-2026-09-12.md` into a plain-language deliverable for Brey and handed it off; all v52 WS-6/local-workflow context below remains authoritative.
BREY_FACING_ENCARGOS_EXPLAINER: https://claude.ai/code/artifact/05008afd-b2f9-4dbc-9ea4-54d73b2c1b2a — private Claude Artifact, plain-language restatement of `brief-design-ws6-cutouts-2026-09-12.md`'s 3 encargos for Brey (non-technical). Not in git; if it needs to be regenerated or updated, redo the translation from the technical brief, don't treat this URL as the source of truth.
PRIOR_LAST_ACTOR_CONTEXT: Code — merged the local Design workflow (PR #173) and configured the local environment
PRIOR_LAST_ACTION: Brey asked to review/merge `chore/local-design-workflow` (PR #173, opened by OMNI — adds `docs/operations/AROMIA_LOCAL_DESIGN_WORKFLOW.md` + `local:infra`/`local:api`/`local:web`/`local:verify`/`local:infra:down` npm scripts) and then configure the local environment end-to-end so Design/Cowork/ChatGPT can iterate at `localhost:3000` without further manual steps. Reviewed the diff (doc + 5 npm scripts + relay bump only, no destructive commands, no secrets), confirmed CI green (`web`, `code-and-governance`, `api`, `catalog` all PASS), squash-merged, pulled `main` locally (`fe8d1e9..9027f17`). Then ran the setup: `npm install` at root + `apps/api` + `apps/web`; created `apps/api/.env` from example (`ADMIN_API_TOKEN` aligned with `apps/web/.env.local`'s); `npm run local:infra` (Postgres 16 + Redis 7 via Docker, fresh volume `aromia-lab_aromia_postgres_data`); `npm run local:web` verified serving real content at `http://localhost:3000` (Home 200 with correct `<title>`, `/catalogo/santal-33-edp` 200, `/magazine` 200, `/buscar` 200 — all against the real production API, see below). See `LOCAL_DESIGN_WORKFLOW` above for the one real gap found (local Postgres can't run the full catalog migration/seed chain — no local snapshot of the real 125-perfume catalog exists) and the fallback used. Unrelated, prior turn's work unchanged: `image-toolkit` (OMNI) connected in the previous Code session closed the D-11 blocker from v48/decision #109. Ran the full pipeline (`remove_background` → `autotrim_image` → `resize_image` → `convert_image`) for the 10 slugs of `docs/images/OMNI-PILOT-2026-09-11.md`, classified A/B/C/D per the handoff (§5). Result, with Brey's explicit sign-off: **7 slugs A** (`bleu-de-chanel-edp`, `chanel-no5-edp`, `santal-33-edp`, `aventus`, `molecule-01`, `acqua-di-gio-edt`, `nishane-hacivat`) now have real `.webp` cutouts in `apps/web/public/perfumes/cutouts/` and are registered in `PERFUME_CUTOUTS` (`apps/web/src/lib/perfumeCutouts.ts`) — `ProductImage.tsx` serves them cutout-first, verified live. **1 slug B** (`oud-wood`) has a correct alpha-channel cutout but a thin edge artifact at the cap/neck join — file kept in the repo, deliberately NOT registered, needs edge cleanup before it can join the approved set. **2 slugs C** (`baccarat-rouge-540-edp`, `1-million`) are box-in-photo composition problems — **Brey explicitly decided to leave them with their box for now**, no re-sourcing, no AI generation; this is a taken decision, not an open blocker. Collateral fix: `nishane-hacivat`'s source (`cdn0.woolworths.media`) was returning HTTP 403 — re-sourced via Amazon (`m.media-amazon.com/images/I/41bBN+h5-DL._SL1500_.jpg`, product `B07TTCL5HY`, Brey confirmed it's the same product by pasting the real Amazon listing URL), and its `imagen_url` was updated in the production Postgres via `PATCH /api/admin/perfumes/44` (same admin mechanism as decision #106). Full per-slug detail, including a reusable note on a `get_result_base64` silent-truncation issue (mitigated by lowering pipeline output size, root cause not found), in `docs/images/OMNI-PILOT-RESULTADOS-2026-09-12.md`. Merged as [PR #170](https://github.com/francoisbowman-cloud/aromia-lab/pull/170) — CI green (web test+lint+typecheck+build, api, code-and-governance all PASS), squash-merged to `main`, Railway auto-deployed both services, verified live against `aromialab.com` (200 OK on the new cutout assets, `imagen_url` confirmed updated via the public API). `ticket-diseno-aromia-punta-a-punta.md` D-11 marked resolved; `ESTADO-aromia.md` decision #110.
ACTIVE_OBJECTIVE: Implementation of `handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md` is done and in [PR #178](https://github.com/francoisbowman-cloud/aromia-lab/pull/178), OPEN. Remaining before merge: OMNI desktop/mobile regression QA (handoff acceptance criteria), then Brey's review/merge — no auto-merge, `main` requires PR review. This focus does not erase the existing `SUBBATCH_01_RUNTIME_P0`, `MAGAZINE_ARTICLE_HERO_CONTRAST_P1`, WS-6 Design work, cutout QA/traceability or editorial Art Direction backlog; those remain open, not silently resolved.
ACTIVE_BRANCH: main; Discovery/Search batch is on `feat/discovery-search-bounded-queries` (PR #178, not yet merged)
DISCOVERY_SEARCH_ARCHITECTURE_HANDOFF: handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md — IMPLEMENTED, PR #178 OPEN (awaiting OMNI QA + merge)
SITES_PLATFORM_DECISION: DO_NOT_MIGRATE — Sites may support isolated prototypes or microsites only; canonical product/data/deployment remain in the existing Aromia stack
...
LOCAL_DESIGN_WORKFLOW: docs/operations/AROMIA_LOCAL_DESIGN_WORKFLOW.md — local = workshop; GitHub = source of truth; Railway = production. Root commands: local:infra, local:api, local:web, local:verify, local:infra:down. **Merged as PR #173 and set up locally 2026-09-12** — `npm run local:infra` (Postgres+Redis Docker), `apps/api/.env` and `apps/web/.env.local` created, `npm run local:web` verified serving real content on `http://localhost:3000` (Home, `/catalogo/[slug]`, `/magazine`, `/buscar` all 200). **Real gap found, not routed around silently:** `npm --prefix apps/api run migrate` on a fresh local Postgres fails at migration `016_restore_and_assert_published_catalog_count.sql` (`CATALOG_ROW_COUNT_UNDERFLOW: expected at least 125 total rows, found 0`) — migrations 011–020 are one-time production data-integrity fixes/gates baked into the migrations directory, not schema-only; they assume the real 125-perfume catalog already exists and there is no local seed/snapshot that satisfies them (`seed.ts` only has the old 50-row CSV). Local Postgres is therefore usable for schema-only work (migrated cleanly through 015) but **not** for running `local:api` against a full local catalog. Practical fallback used, matching the workflow's own "public API reads when sufficient" rule and the pre-existing pattern in `CLAUDE.md`: `apps/web/.env.local` → `NEXT_PUBLIC_API_URL=https://api-production-fe2f.up.railway.app` (read-only GETs against the real production API, no admin/write calls). If a fully offline local API is ever needed, someone has to build a sanitized production-catalog snapshot/import script first — not attempted here, out of scope for this turn. **Runtime note for a clean session:** as of this relay update, the Postgres/Redis Docker containers and the `next dev` process (`localhost:3000`) from this setup are still running in this machine's session — a fresh Code session should check with `docker ps` / `curl localhost:3000` before assuming they need to be started again, and should NOT assume they'll still be up if the machine restarts or the terminal session ends.
...
NEXT_ACTOR: Code — PRIMARY for the Publisher-designated Discovery/Search architecture batch. ChatGPT/Art Direction may continue the already-ready editorial/WS-6 streams in parallel, but must not alter or duplicate the data-flow implementation scope.
NEXT_ACTION: Read `handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md`; execute Phase A measurements and compact query-contract definition, then continue through Search, Discovery, caching/resilience, tests, before/after evidence, OMNI desktop/mobile regression QA and PR. Do not wait for a Sites migration or create a second application. Preserve the known production P0/P1 and WS-6 states in the closing relay.
