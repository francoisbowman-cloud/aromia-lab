...
LAST_ACTOR: ChatGPT / Sites evaluation — audited whether Aromia should adopt Sites and converted the Publisher's selected priority into a durable Code handoff
LAST_ACTION: Evaluated the live Aromia surfaces and current repository architecture. Decision: keep GitHub + Next.js + Express/Postgres/Redis + Railway + OMNI as the canonical stack; do not duplicate or migrate the magazine, catalog, Discovery, search or CMS into Sites. The Publisher explicitly selected point 3 as the next focus: Discovery and search must stop transporting the complete perfume catalog to the browser. Added `handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md` with current-code evidence, phased implementation, privacy constraints, acceptance criteria and QA/PR boundaries. No application code or production state changed in this turn.
...
ACTIVE_OBJECTIVE: PUBLISHER-DESIGNATED FOCUS — execute `handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md`. Replace full-catalog browser hydration in `/buscar` and `/descubrir` with bounded server-backed queries and compact purpose-specific data while preserving the current visual direction, URLs, analytics semantics and local-only olfactory-profile privacy contract. Measure before/after response size, serialized data and interaction responsiveness from the same environment. This focus does not erase the existing `SUBBATCH_01_RUNTIME_P0`, `MAGAZINE_ARTICLE_HERO_CONTRAST_P1`, WS-6 Design work, cutout QA/traceability or editorial Art Direction backlog; preserve those states and do not silently mark them resolved.
ACTIVE_BRANCH: main; Code should create a dedicated branch for the Discovery/Search architecture batch
DISCOVERY_SEARCH_ARCHITECTURE_HANDOFF: handoffs/AROMIA_CODE_DISCOVERY_SEARCH_ARCHITECTURE_2026-09-20.md — READY_FOR_CODE / PUBLISHER-DESIGNATED PRODUCT FOCUS
