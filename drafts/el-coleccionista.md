...
Narrative purpose: Establish the opening image the text describes — a shelf or drawer that has run out of room, before any argument or data appears.
Emotional objective: Quiet recognition, not spectacle. The reader should feel "that's my shelf," not be impressed by abundance.
Authenticity constraints: Must read as a real, lived-in collection — mixed brands, mixed bottle shapes, some empty space ignored rather than a styled full symmetric display. Nothing that looks like a retail shelf or an ad set.
Relationship to surrounding text: Opens the piece, before the first subhead — functions as the entry door the Voice Bible asks for, not an illustration of a later claim.
Other hard constraints: No text/logos legible on bottles unless real and correctly licensed; avoid implying any single brand endorses the piece.
...
Emotional objective: A slightly dizzying, "oh, it never stopped" feeling — not celebratory, not critical either.
Authenticity constraints: If Le Male's actual flanker lineup is used, bottle shapes/colors must match real releases — no invented variants. Verify current lineup before rendering anything as fact.
Relationship to surrounding text: Sits at the close of this section, after the flanker data and the Le Male example — a visual full stop on the point, not a hero image for the perfume itself.
Other hard constraints: This is not a product shot for conversion — resist making it look like a retail display or a "shop now" moment.
