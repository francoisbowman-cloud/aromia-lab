...
**Corrección 25/08:** el brief original decía escribir en `apps/api/data/articles/*.md`
— es incorrecto, esos archivos no son la fuente real (ver `CLAUDE.md`, la fuente real es
la tabla `articles` de Postgres vía `/admin/magazine`, sin sincronización con esos
`.md`). `brief-apertura-cowork-editorial.md` ya fue corregido para escribir borradores
en `drafts/` en su lugar. Pendiente aparte, no bloqueante: los artículos ya publicados
antes del pivote no tienen el link de afiliado que ahora se requiere — retrofit de baja
prioridad, no frena los artículos nuevos.
