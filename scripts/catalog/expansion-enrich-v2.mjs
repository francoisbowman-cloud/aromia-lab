...
export function requireVerifiedNotesUnavailability(row) {
  ...
  return { ...row, source_does_not_publish_notes: "false", notes_unavailability_guard: "unverified_claim_downgraded_to_unresolved" };
}

export function sanitizeReadinessEvidence(row) {
  return sanitizeExtractedNotes({ ...row, title: row.page_title || row.title || `${row.brand ?? ""} ${row.name ?? ""}`.trim() });
}
...
export function recalibrateCatalogReadiness(draft) {
  ...
  return { ...draft, quality_status: EXPANSION_STATES.AUTO_READY, quality_reason: "catalog_confidence_gate_pass;publication_enrichment_pending" };
}

export function routeEvidenceV2(rows, options = {}) {
  const drafted = rows.map((row) => recalibrateCatalogReadiness(evidenceToDraft(requireVerifiedNotesUnavailability(sanitizeReadinessEvidence(row)), options)));
  return {
    drafted,
...
export function runEnrichmentV2({ batchId = DEFAULT_EXPANSION_BATCH, ...options } = {}) {
  const dir = expansionDir(batchId); const resolved = resolveEvidencePath(dir);
  if (!resolved) return { skipped: true, reason: "no evidence.csv or evidence.auto.csv present", batch_id: batchId };
  ...
  const total = routed.drafted.length; const reviewReasons = {}; const penaltyCounts = {};
  ...
  const report = {
    batch_id: batchId, evidence_source: resolved.source, readiness_contract: "catalog-v2", total,
    auto_ready: routed.autoReady.length, review_required: routed.reviewRequired.length, blocked: routed.blocked.length,
    ...
    publication_complete: publicationComplete, publication_completion_yield: total ? publicationComplete / total : null,
    image_complete: imageComplete, image_completion_yield: total ? imageComplete / total : null,
    affiliate_active: affiliateActive, affiliate_activation_yield: total ? affiliateActive / total : null,
    affiliate_configuration_present: Boolean(options.associateTag ?? process.env.AROMIA_AMAZON_ASSOCIATE_TAG),
    publication_enrichment_is_blocking: false, review_reasons: reviewReasons, review_penalties: penaltyCounts,
  };
  writeFileSync(join(dir, "enrichment-report.json"), JSON.stringify(report, null, 2) + "\n", "utf-8");
  return report;
}

if (isMainModule(import.meta.url)) console.log(JSON.stringify(runEnrichmentV2({ batchId: resolveBatchId() }), null, 2));
