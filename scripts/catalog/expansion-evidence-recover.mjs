...
export function recoverPartialOfficialEvidence(row) {
  ...
  return {
    ...row, harvest_status: "PARTIAL_EVIDENCE", source_url: sourceUrl,
    ...
    evidence_method: "official_html_identity_only", provenance_recovered: "true",
...
export function recoverEvidenceRows(rows) { return rows.map(recoverPartialOfficialEvidence); }

export function runEvidenceRecovery({ batchId = DEFAULT_EXPANSION_BATCH } = {}) {
  const dir = expansionDir(batchId); const path = join(dir, "evidence.auto.csv");
  if (!existsSync(path)) return { skipped: true, reason: "evidence.auto.csv not present", batch_id: batchId };
  ...
  return { batch_id: batchId, total: recovered.length, partial_provenance_recovered: count, production_write: false };
}

if (isMainModule(import.meta.url)) console.log(JSON.stringify(runEvidenceRecovery({ batchId: resolveBatchId() }), null, 2));
