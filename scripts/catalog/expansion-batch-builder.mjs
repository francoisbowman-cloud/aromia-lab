...
export function toCatalogBatchRow(row, date = new Date().toISOString().slice(0, 10)) {
  ...
  return {
    id, slug, brand, name, concentration, gender: pending(row.gender), family: pending(row.family), subfamily: "pending",
    launch_year: pending(row.launch_year), perfumer: pending(row.perfumer), country: pending(row.country), description: "pending",
    ...
    notes_status: pending(row.notes_status), season: "pending", occasion: "pending", longevity: "pending", sillage: "pending",
    price_segment: pending(row.price_segment), price_status: pending(row.price_status), amazon_url: "pending", source_url: pending(row.source_url),
    image_url: "pending", image_source: "pending", affiliate_status: "no-applicable", source_verified: "true",
    data_confidence: confidenceLabel(row.overall_confidence), visual_quality: "not-audited", review_status: "pending",
    seo_title: "pending", seo_description: "pending", status: "draft",
    notes: `Expansion Automation v1; quality_reason=${pending(row.quality_reason)}; confidence=${pending(row.overall_confidence)}; provenance preserved; no production write.`,
...
export function runBatchBuilder() {
  ...
  return { rows: rows.length, output, production_write: false, master_import: false };
}
