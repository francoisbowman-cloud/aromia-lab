...
import { discoverSecondaryEvidence } from "./secondary-discovery.mjs";
...
export function mergeSecondaryEvidence(row, secondary, candidate) {
  if (secondary.status !== "FOUND") return { ...row, secondary_status: secondary.status, secondary_error: secondary.error ?? "" };
  ...
  return {
    ...
    notes_structure: clean(row.notes_structure) !== "UNKNOWN" ? row.notes_structure : (secondary.notes_structure || "UNKNOWN"),
...
export async function runSecondaryEnrichment({ concurrency = 5 } = {}) {
  ...
  const resolved=await mapConcurrent(targets,concurrency,async(row)=>{ const candidate=byId.get(row.candidate_id)??row; const secondary=await discoverSecondaryEvidence(candidate); return mergeSecondaryEvidence(row,secondary,candidate); });
  ...
  const report={total:evidence.length,targets:targets.length,secondary_found:resolved.filter((r)=>r.secondary_status==="FOUND").length,secondary_not_found:resolved.filter((r)=>r.secondary_status!=="FOUND").length,gender_filled:resolved.filter((r)=>clean(r.gender)).length,notes_resolved:resolved.filter((r)=>clean(r.notes_structure)&&r.notes_structure!=="UNKNOWN").length,production_write:false};
