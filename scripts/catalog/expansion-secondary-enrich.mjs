...
export function mergeSecondaryEvidence(row, secondary, candidate) {
  ...
  const expected = canonical(candidate.concentration); const observed = canonical(secondary.concentration);
  const concentrationOkay = !observed || !expected || observed === expected; const existingOfficial = bool(row.official_source);
  const notesResolved = secondary.notes_structure && secondary.notes_structure !== "UNKNOWN";
  return {
    ...row, source_url: mergeUrls(row.source_url, secondary.source_url),
    identity_confirmed: bool(row.identity_confirmed) || bool(secondary.identity_confirmed) ? "true" : "false",
    official_source: existingOfficial ? "true" : "false", secondary_source: "true",
    gender: clean(row.gender) || clean(secondary.gender), concentration: clean(row.concentration) || (concentrationOkay ? clean(secondary.concentration) : ""),
    launch_year: clean(row.launch_year) || clean(secondary.launch_year),
    top_notes: clean(row.top_notes) || (notesResolved ? clean(secondary.top_notes) : ""), middle_notes: clean(row.middle_notes) || (notesResolved ? clean(secondary.middle_notes) : ""),
    base_notes: clean(row.base_notes) || (notesResolved ? clean(secondary.base_notes) : ""), accords: clean(row.accords) || (notesResolved ? clean(secondary.accords) : ""),
    ...
    secondary_status: "FOUND", curated_source_url: secondary.source_url,
    secondary_concentration_observed: clean(secondary.concentration), secondary_concentration_match: concentrationOkay ? "true" : "false",
    secondary_launch_year_observed: clean(secondary.launch_year), relation_ambiguous: concentrationOkay ? (row.relation_ambiguous || "false") : "true",
    evidence_method: [clean(row.evidence_method), clean(secondary.evidence_method)].filter(Boolean).join("+"), secondary_error: "",
  };
}
...
export async function runSecondaryEnrichment({ concurrency = 5, batchId = DEFAULT_EXPANSION_BATCH } = {}) {
  ...
  const resolved=await mapConcurrent(targets,concurrency,async(row)=>{
    ...
    if(!secondary||secondary.status!=="FOUND"){
      const searched=await discoverSecondaryEvidence(candidate); if(searched.status==="FOUND") secondary=searched; else if(!secondary) secondary=searched;
    }
    return mergeSecondaryEvidence(row,secondary,candidate);
  });
  const resolvedById=new Map(resolved.map((r)=>[r.candidate_id,r])); const merged=evidence.map((r)=>resolvedById.get(r.candidate_id)??r);
  writeFileSync(evidencePath,stringifyCsv(merged,{header:true}),"utf-8");
  const report={batch_id:batchId,total:evidence.length,targets:targets.length,curated_map_entries:sourceMap.size,
    secondary_found:resolved.filter((r)=>r.secondary_status==="FOUND").length,secondary_not_found:resolved.filter((r)=>r.secondary_status!=="FOUND").length,
    gender_filled:resolved.filter((r)=>clean(r.gender)).length,concentration_filled:resolved.filter((r)=>clean(r.concentration)).length,
    ...
    concentration_conflicts:resolved.filter((r)=>r.secondary_concentration_match==="false").length,production_write:false};
  writeFileSync(join(dir,"secondary-enrichment-report.json"),JSON.stringify(report,null,2)+"\n","utf-8"); return report;
}
if(isMainModule(import.meta.url)) runSecondaryEnrichment({batchId:resolveBatchId(),concurrency:Number(process.env.AROMIA_SECONDARY_CONCURRENCY||5)}).then((r)=>console.log(JSON.stringify(r,null,2))).catch((e)=>{console.error(e);process.exitCode=1;});
