...
export function classifyCandidate(candidate, universe) {
  ...
  if (parts.concentration && universe.exact.has(exactIdentityKey(candidate))) return { relation: "EXISTING", state: EXPANSION_STATES.BLOCKED };
...
export function selectCandidates(poolRows, universe, { limit = 100, quotas = DEFAULT_QUOTAS } = {}) {
  ...
  const seen = new Set();
  const eligible = evaluated.filter((r) => r.state !== EXPANSION_STATES.BLOCKED).filter((r) => {
    const key = exactIdentityKey(r); if (seen.has(key)) return false; seen.add(key); return true;
  ...
  for (const [stratum, quota] of Object.entries(quotas)) {
    for (const row of eligible.filter((r) => clean(r.stratum) === stratum).slice(0, quota)) {
      ...
      if (!selectedKeys.has(key) && selected.length < limit) { selected.push(row); selectedKeys.add(key); }
    }
  }
...
export function metrics(rows) {
  ...
  return { total, counts, autoPreparationYield: total ? counts.AUTO_READY / total : null, humanReviewBurden: total ? counts.REVIEW_REQUIRED / total : null, blockedRate: total ? counts.BLOCKED / total : null };
}
...
export function runExpansion({ limit = 100 } = {}) {
  const expansionDir = join(REPO_ROOT, "catalog", "expansion");
  const poolPaths = ["candidate-pool-v1a.csv", "candidate-pool-v1b.csv", "candidate-pool-v1c.csv"].map((name) => join(expansionDir, name));
  ...
  const outputDir = join(expansionDir, "batch-003");
