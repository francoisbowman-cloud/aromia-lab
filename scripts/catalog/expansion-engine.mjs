...
export function identityParts(row) { const brand = fold(row.brand ?? row.marca); let rawName = clean(row.name ?? row.nombre); let concentration = clean(row.concentration); if (!concentration && rawName) { const x = extractConcentrationFromName(rawName); rawName = x.baseName; concentration = x.concentration ?? ""; } const n = normalizeConcentration(concentration); return { brand, name: fold(rawName), concentration: fold(n.value) }; }
...
export function scoreCandidate(candidate, relation, gapAnalysis) {
  let score = Number(candidate.priority || 50);
  if (relation === "NEW") score += 10;
  if (relation === "RELATED_VARIANT") score += 5;
  score += brandGapBonus(candidate, gapAnalysis);
  if (clean(candidate.official_domain)) score += 8;
  if (clean(candidate.concentration)) score += 4;
  if (clean(candidate.launch_year)) score += 2;
  if (clean(candidate.difficulty).toLowerCase() === "hard") score -= 3;
  return score;
}

export function selectCandidates(poolRows, universe, { limit = 100, quotas = DEFAULT_QUOTAS, gapAnalysis = null } = {}) {
  const evaluated = poolRows.map((candidate, index) => { const { relation, state } = classifyCandidate(candidate, universe); return { ...candidate, relation, state, gap_bonus: brandGapBonus(candidate, gapAnalysis), score: scoreCandidate(candidate, relation, gapAnalysis), _index: index }; });
  const seen = new Set();
...
export function buildDiscoveryTask(candidate) { const domain = clean(candidate.official_domain); const concentration = clean(candidate.concentration); const label = `${clean(candidate.brand)} ${clean(candidate.name)}${concentration ? ` ${concentration}` : ""}`.trim(); return { candidate_id: clean(candidate.candidate_id), identity: label, official_domain: domain, primary_query: `${label} official fragrance notes`, fallback_query: `${label} perfume notes launch year perfumer`, preferred_source: domain ? `https://${domain}` : "", provenance_required: "true", extraction_state: "PENDING_DISCOVERY" }; }

export function computeConfidence(evidence = {}) {
  const reasons = []; const penalties = [];
  const identity = evidence.identityConfirmed ? 1 : 0.55; const source = evidence.officialSource ? 1 : evidence.secondarySource ? 0.72 : 0.35; const notes = evidence.notesPublished ? 1 : evidence.notesUnavailableConfirmed ? 0.82 : 0.45; const metadata = evidence.metadataFields >= 4 ? 1 : evidence.metadataFields >= 2 ? 0.75 : 0.5; const relation = evidence.relationUnambiguous === false ? 0.45 : 1;
  if (evidence.identityConfirmed) reasons.push("identity_confirmed"); else penalties.push("identity_unconfirmed"); if (evidence.officialSource) reasons.push("official_source"); else if (evidence.secondarySource) reasons.push("secondary_source"); else penalties.push("weak_source"); if (evidence.notesPublished) reasons.push("notes_published"); else if (evidence.notesUnavailableConfirmed) reasons.push("source_does_not_publish_notes"); else penalties.push("notes_unresolved"); if (relation < 1) penalties.push("relation_ambiguous");
...
export function qualityGate({ confidence, blockingConflict = false, provenanceCount = 0 } = {}) { if (blockingConflict) return { state: EXPANSION_STATES.BLOCKED, reason: "blocking_conflict" }; if (!confidence || provenanceCount < 1) return { state: EXPANSION_STATES.REVIEW_REQUIRED, reason: "missing_provenance" }; if (confidence.relation_confidence < 0.7) return { state: EXPANSION_STATES.REVIEW_REQUIRED, reason: "ambiguous_relation" }; if (confidence.source_confidence < 0.7) return { state: EXPANSION_STATES.REVIEW_REQUIRED, reason: "source_confidence_below_threshold" }; if (confidence.notes_confidence < 0.8) return { state: EXPANSION_STATES.REVIEW_REQUIRED, reason: "notes_unresolved" }; if (confidence.overall_confidence >= 0.82) return { state: EXPANSION_STATES.AUTO_READY, reason: "confidence_gate_pass" }; return { state: EXPANSION_STATES.REVIEW_REQUIRED, reason: "confidence_below_threshold" }; }
...
export function runExpansion({ limit = 100 } = {}) {
  ...
  const currentRows = safeRead(currentPath); const batchRows = [...safeRead(b1Path), ...safeRead(b2Path)]; const masterRows = safeRead(masterPath); const knownRows = [...currentRows, ...batchRows, ...masterRows];
  const poolRows = poolPaths.flatMap(readCsv); const universe = buildKnownUniverse({ currentRows, batchRows, masterRows }); const gapAnalysis = analyzeCatalogGaps(knownRows); const { selected, evaluated } = selectCandidates(poolRows, universe, { limit, gapAnalysis }); const discovery = selected.map(buildDiscoveryTask);
