...
function extractLaunchYear(text){ const head=String(text??"").slice(0,9000); const m=head.match(/(?:was launched in|launched in|se lanzó en|fue lanzad[oa] en)\s*((?:19|20)\d{2})/i); return m?.[1]??""; }
function extractNotes(text){ const t=String(text??""); const top=t.match(/Top notes? (?:are|is) ([^.]{3,340})\./i)?.[1]??t.match(/Las Notas de Salida son ([^.]{3,340})\./i)?.[1]??""; const mid=t.match(/middle notes? (?:are|is) ([^.]{3,340})\./i)?.[1]??t.match(/Las Notas de Corazón son ([^.]{3,340})\./i)?.[1]??""; const base=t.match(/base notes? (?:are|is) ([^.]{3,340})\./i)?.[1]??t.match(/Las Notas de Fondo son ([^.]{3,340})\./i)?.[1]??""; const features=t.match(/The fragrance features ([^.]{3,440})\./i)?.[1]??t.match(/La fragancia (?:contiene|presenta) ([^.]{3,440})\./i)?.[1]??""; const split=(v)=>String(v).split(/,| and | y /i).map((x)=>x.trim()).filter(Boolean).join(";"); if(top||mid||base)return {top_notes:split(top),middle_notes:split(mid),base_notes:split(base),accords:"",notes_structure:(top&&mid&&base)?"PYRAMID":"PARTIAL"}; if(features)return {top_notes:"",middle_notes:"",base_notes:"",accords:split(features),notes_structure:"FLAT"}; return {top_notes:"",middle_notes:"",base_notes:"",accords:"",notes_structure:"UNKNOWN"}; }
