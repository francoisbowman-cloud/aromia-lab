...
function splitList(value) {
  return clean(value) ? clean(value).split(";").map((x) => x.trim()).filter(Boolean) : [];
}
...
export function evidenceToDraft(row) {
  ...
  return {
    ...
    top_notes: top.join(";"),
    middle_notes: middle.join(";"),
    base_notes: base.join(";"),
    accords: accords.join(";"),
    price_segment: clean(row.price_segment),
    price_status: derivePriceStatus(clean(row.price_segment)),
    source_url: sourceUrls.join(";"),
    notes_structure: notesStructure,
    catalog_relation: clean(row.catalog_relation) || "NEW",
    quality_status: gate.state,
    quality_reason: gate.reason,
    overall_confidence: confidence.overall_confidence,
    identity_confidence: confidence.identity_confidence,
    source_confidence: confidence.source_confidence,
    notes_confidence: confidence.notes_confidence,
    metadata_confidence: confidence.metadata_confidence,
    relation_confidence: confidence.relation_confidence,
    confidence_reasons: confidence.confidence_reasons.join(";"),
    confidence_penalties: confidence.confidence_penalties.join(";"),
  };
}
...
function writeCsv(path, rows) {
  ...
  if (!rows.length) { writeFileSync(path, "", "utf-8"); return; }
