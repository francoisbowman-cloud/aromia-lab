...
export async function harvestCandidateV2(candidate, options = {}) {
  ...
  const discovered = await discoverOfficialUrlsV2(candidate, options);
  const discoveredUrls = discovered.status === "FOUND" ? discovered.urls : [];
  const seen = new Set();
  const urls = [...preferred, ...discoveredUrls].filter((hit) => { if (seen.has(hit.url)) return false; seen.add(hit.url); return true; });
  const base = {
  ...
  for (const hit of urls) {
    try {
      ...
      if (expected && extracted !== expected) { errors.push(`concentration_conflict:${candidate.concentration}->${evidence.concentration}:${hit.url}`); continue; }
      ...
      const noNotes = evidence.notes_structure === "UNKNOWN";
      return {
        ...base, concentration: evidence.concentration, gender: evidence.gender, family: evidence.family,
        launch_year: evidence.launch_year, perfumer: evidence.perfumer, description: publication.description,
        harvest_status: "HARVESTED", source_url: evidence.source_url, identity_confirmed: "true", official_source: "true",
        source_does_not_publish_notes: noNotes ? "true" : "false", top_notes: evidence.top_notes,
        middle_notes: evidence.middle_notes, base_notes: evidence.base_notes, accords: evidence.accords,
        notes_structure: evidence.notes_structure, page_title: evidence.title, page_description: evidence.description,
        image_url: publication.image_url, image_source: publication.image_source, amazon_url: publication.amazon_url,
        affiliate_status: publication.affiliate_status, visual_quality: publication.visual_quality,
        ...
        evidence_method: evidence.evidence_method, identity_token_coverage: identity.coverage.toFixed(2),
        discovery_score: hit.score, discovery_method: hit.method || "discovery_v2",
...
export async function mapConcurrent(items, limit, fn, onProgress = null) {
  ...
  async function worker() { while (true) { const index = cursor++; if (index >= items.length) return; results[index] = await fn(items[index], index); completed += 1; onProgress?.(completed, items.length, results[index], items[index]); } }
...
export function summarizeHarvest(rows) {
  ...
  const withImages = harvested.filter((r) => r.image_url); const withDescriptions = harvested.filter((r) => r.description); const withCommerceLinks = harvested.filter((r) => r.amazon_url);
  return { total: rows.length, counts, failure_causes: causes, harvested: harvested.length, harvested_rate: rows.length ? harvested.length / rows.length : null,
    explicit_notes: explicitNotes.length, explicit_notes_rate: rows.length ? explicitNotes.length / rows.length : null,
    verified_source_no_notes: noNotesPublished.length, critical_complete: criticalComplete.length, critical_complete_rate: rows.length ? criticalComplete.length / rows.length : null,
...
export async function runHarvestV2({ concurrency = DEFAULT_CONCURRENCY, limit = 100, options = {} } = {}) {
  const dir = join(REPO_ROOT, "catalog", "expansion", "batch-003"); const manifestPath = join(dir, "candidate-manifest.csv");
  if (!existsSync(manifestPath)) return { skipped: true, reason: "candidate-manifest.csv not present; run expand:v2 first" };
  const candidates = readCsv(manifestPath).slice(0, limit);
  const rows = await mapConcurrent(candidates, concurrency, (candidate) => harvestCandidateV2(candidate, options), (completed, total, row, candidate) => { if (completed === 1 || completed % 10 === 0 || completed === total) console.log(`[harvest-v2] ${completed}/${total} latest=${candidate.candidate_id}:${row.harvest_status}`); });
  writeCsv(join(dir, "harvest-results.csv"), rows); writeCsv(join(dir, "evidence.auto.csv"), rows);
  const report = summarizeHarvest(rows); writeFileSync(join(dir, "harvest-report.json"), JSON.stringify(report, null, 2) + "\n", "utf-8"); return report;
}

if (isMainModule(import.meta.url)) runHarvestV2({ limit: Number(process.argv[2] || 100), concurrency: Number(process.env.AROMIA_HARVEST_CONCURRENCY || DEFAULT_CONCURRENCY) }).then((r) => console.log(JSON.stringify(r, null, 2))).catch((e) => { console.error(e); process.exitCode = 1; });
