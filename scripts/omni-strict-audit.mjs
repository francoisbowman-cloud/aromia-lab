...
await requireText("apps/web/src/app/catalogo/[slug]/page.tsx", [
  ["pdp_missing_not_found", /notFound\(\)/],
  ["pdp_missing_canonical", /canonical:\s*`\/catalogo\/\$\{perfume\.slug\}`/],
  ["pdp_missing_product_jsonld", /application\/ld\+json/],
  ["pdp_missing_hero", /<HeroHeader/],
  ["pdp_missing_sensory_anatomy", /<SkinEvolution/],
  ["pdp_missing_performance", /<PerformanceBars/],
  ["pdp_missing_story", /<EditorialMood/],
  ["pdp_missing_commerce", /<PriceTable/],
  ["pdp_missing_community", /<CommunityReviews/],
]);
...
await requireText("apps/api/src/db/applyCatalogCompleteness.ts", [
  ["amazon_enrichment_should_be_degraded_not_fatal", /AMAZON_CATALOG_COMPLETION_DEGRADED/],
]);

await requireText("apps/web/preflight-catalog.mjs", [
  ["web_preflight_must_not_kill_availability", /startup will continue/],
  ["web_preflight_must_exit_zero", /process\.exit\(0\)/],
]);

await requireText("apps/web/src/lib/analytics.ts", [
  ["analytics_must_be_guarded", /typeof window|window\./],
]);
